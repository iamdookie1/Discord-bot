...
tabs.forEach((tab) => {
  tab.addEventListener("click", () => {
    ...
    if (tab.dataset.tab === "nsfw") loadNsfwCommands();
  });
});
...
const nsfwCmdList = document.getElementById("nsfwCmdList");
const nsfwCmdEmpty = document.getElementById("nsfwCmdEmpty");
const nsfwCmdName = document.getElementById("nsfwCmdName");
const nsfwCmdDescription = document.getElementById("nsfwCmdDescription");
const createNsfwCmdBtn = document.getElementById("createNsfwCmdBtn");
const nsfwCreateMsg = document.getElementById("nsfwCreateMsg");

const MAX_NSFW_MEDIA = 5;

function renderNsfwCmdItem(c) {
  const item = document.createElement("div");
  item.className = "rp-cmd-item";
  ...
  row.appendChild(renderToggle(c.name, c.enabled, async (checked) => {
    await api(`/api/nsfw/commands/${encodeURIComponent(c.name)}/toggle`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ enabled: checked }),
    });
  ...
  const info = document.createElement("div");
  info.className = "cmd-info";
  const mediaCount = c.media.length ? `${c.media.length} item${c.media.length === 1 ? "" : "s"}` : "no media yet";
  info.innerHTML = `
    <span class="cmd-name mono">!${escapeHtml(c.name)}</span>
    <span class="cmd-desc">${escapeHtml(c.description || "")} &middot; ${mediaCount}</span>
  `;
  row.appendChild(info);
  ...
  deleteBtn.addEventListener("click", async () => {
    deleteBtn.disabled = true;
    await api(`/api/nsfw/commands/${encodeURIComponent(c.name)}`, { method: "DELETE" });
    loadNsfwCommands();
  });
  actions.appendChild(deleteBtn);
  ...
  const mediaInputs = [];
  for (let i = 0; i < MAX_NSFW_MEDIA; i++) {
    const input = document.createElement("input");
    input.type = "text";
    input.className = "field-input mono rp-gif-input";
    input.placeholder = `Media URL ${i + 1}`;
    input.value = c.media[i] || "";
  ...
  saveBtn.addEventListener("click", async () => {
    saveBtn.disabled = true;
    const media = mediaInputs.map((inp) => inp.value.trim());
    const data = await api(`/api/nsfw/commands/${encodeURIComponent(c.name)}/media`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function loadNsfwCommands() {
  const cmds = await api("/api/nsfw/commands");
  nsfwCmdEmpty.style.display = cmds.length ? "none" : "block";
  nsfwCmdList.innerHTML = "";
  cmds.forEach((c) => nsfwCmdList.appendChild(renderNsfwCmdItem(c)));
}

createNsfwCmdBtn.addEventListener("click", async () => {
  const name = nsfwCmdName.value.trim().toLowerCase();
  const description = nsfwCmdDescription.value.trim();

  if (!name) {
    setMsg(nsfwCreateMsg, "Give the command a name.", "error");
    return;
  }

  createNsfwCmdBtn.disabled = true;
  setMsg(nsfwCreateMsg, "Saving...", "");
  const data = await api("/api/nsfw/commands", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ name, description, media: [] }),
  });
  createNsfwCmdBtn.disabled = false;

  if (data.ok) {
    setMsg(nsfwCreateMsg, "Created — add media to it below.", "success");
    nsfwCmdName.value = "";
    nsfwCmdDescription.value = "";
    loadNsfwCommands();
  } else {
    setMsg(nsfwCreateMsg, data.error || "Couldn't create that command.", "error");
  }
});
